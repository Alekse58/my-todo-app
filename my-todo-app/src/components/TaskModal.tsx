...
import { STATUS_TASK_TEXT } from '@/blueprint/constante/task';
import { ETaskStatus, ITaskUpdate } from '@/blueprint/types/TaskTypes';
  ...
  return (
    <Dialog.Root open={ isOpen } onOpenChange={ onClose }>
      ...
      <Dialog.Content className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-md p-4 w-96'>
        ...
        <Dialog.Description className='mt-2 text-gray-700'>Заполните информацию о задаче.</Dialog.Description>
        <form onSubmit={ handleSubmit } className='mt-4'>
          ...
          <div className='mt-6'>
            <button type='submit' className='px-4 py-2 bg-blue-500 text-white rounded-xl'>Создать задачу</button>
            <button type='button' onClick={ onClose } className='ml-2 px-4 py-2 bg-gray-300 rounded-xl'>Отмена</button>
          </div>
        </form>
      </Dialog.Content>
    </Dialog.Root>
